...
class PointerLockController extends EventDispatcher {
    constructor(camera, domElement) {
        ...
        function onMouseMove(event) {
            ...
            // move upVector to 0,1,0
            _up.copy(camera.position).multiplyScalar(-1).normalize();
            _pos.copy(camera.position);

            camera.position.set(0, 0, 0);

            camera.applyQuaternion(
                _quaternion.setFromUnitVectors(_up, _vector.set(0, 1, 0))
            );
            ...
            // move 0,1,0 to up
            camera.applyQuaternion(
                _quaternion.setFromUnitVectors(_vector.set(0, 1, 0), _up)
            );
        ...
        function onPointerlockChange() {
            if (
                scope.domElement.ownerDocument.pointerLockElement ===
                scope.domElement
            ) {
